...
def seed_orders(checkouts_per_buyer, max_items_per_cart, fulfilled_ratio, seed):
    random.seed(seed)
    ...
    print(f"\n{'Stopped' if stop_requested else 'Done'} — "
          f"{total_checkouts} checkouts, {total_orders} orders, {total_items} items")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--checkouts-per-buyer", type=int, default=5,
                        help="Number of checkouts each buyer performs (default: 5). "
                             "Each checkout produces 1+ orders depending on how many sellers are involved.")
    parser.add_argument("--max-items-per-cart", type=int, default=6,
                        help="Maximum distinct products per cart (default: 6)")
    ...
    seed_orders(args.checkouts_per_buyer, args.max_items_per_cart,
                args.fulfilled_ratio, args.seed)
